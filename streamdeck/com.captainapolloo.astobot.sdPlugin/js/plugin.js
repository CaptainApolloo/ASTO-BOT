...
/**
 * Alle sichtbaren ASTO-BOT-Tasten: context -> action.
 *
 * Gebraucht wird das, damit JEDE Taste den Verbindungszustand anzeigen kann —
 * farbig wenn ASTO-BOT erreichbar ist, grau wenn nicht. So sieht man am ganzen
 * Deck auf einen Blick, ob die App laeuft, ohne erst eine Taste zu druecken.
 */
...
function refreshKeys() {
  // Zustand 0 = offline (grau), 1 = verbunden (farbig) — Reihenfolge wie im
  // Manifest. Nur die Connection-Taste bekommt zusaetzlich den Text, bei den
  // uebrigen wuerde er den vom Nutzer gesetzten Titel ueberschreiben.
  allContexts.forEach(function (action, context) {
...
function connectElgatoStreamDeckSocket(inPort, inPluginUUID, inRegisterEvent, inInfo) {
  ...
  sdSocket.onmessage = function (ev) {
    ...
    if (event === 'keyUp') {
      ...
      // gewachsene Wartezeit (bis 15 s) stehen, und der naechste automatische
      // Versuch käme erst spaet — genau dann, wenn man ASTO-BOT gerade
      // gestartet hat und nicht warten will.
      if (action.endsWith('.status')) {
        reconnectDelay = 1000;
      }
      ...
      // mit zwei Zustaenden von Haus aus als Umschalter und wechselt beim
      // Druck selbsttaetig — die Taste wuerde grau, obwohl die Verbindung
      // steht. Im Manifest ist das ueber DisableAutomaticStates abgeschaltet;
      // das hier greift zusaetzlich, falls die Angabe von einer aelteren
      // Stream-Deck-Fassung ignoriert wird. Der zweite Aufruf faengt das
      // Neuzeichnen ab, nachdem das Haekchen bzw. Warndreieck verschwindet.
      setState(context, astoConnected ? 1 : 0);
      setTimeout(function () { setState(context, astoConnected ? 1 : 0); }, 1400);
      return;
    }

    if (event === 'willAppear') {
      // Jede ASTO-BOT-Taste mitfuehren, nicht nur die Connection-Taste.
      allContexts.set(context, action);
      setState(context, astoConnected ? 1 : 0);
      if (action.endsWith('.status')) {
        setTitle(context, astoConnected ? '' : 'offline');
      }
